  ...
  try {
    if (req.method === 'GET') {
      ...
      if (id) {
        ...
        const sections = await sql`SELECT * FROM cote_daily_sections WHERE worksheet_id=${id} ORDER BY position`;
        const secIds = sections.map(s=>s.id);
        let rows = [];
        if (secIds.length) {
          rows = await sql`SELECT * FROM cote_daily_rows WHERE section_id = ANY(${secIds}) ORDER BY position`;
        }
    ...
    if (req.method === 'POST') {
      ...
      const { name } = body;
      const ws = await sql`INSERT INTO cote_daily_worksheets (name) VALUES (${name}) RETURNING *`;
    ...
    if (req.method === 'PUT') {
      ...
      await sql`DELETE FROM cote_daily_rows WHERE section_id IN (SELECT id FROM cote_daily_sections WHERE worksheet_id=${id})`;
      ...
      for (let i=0;i<sections.length;i++) {
        ...
        for (let j=0;j<s.rows.length;j++) {
          const r = s.rows[j];
          await sql`
            INSERT INTO cote_daily_rows (section_id, bus_number_default, pickup_default, dropoff_default,
              pickup_time_default, ds_in_am_default, ns_out_am_default, ds_out_pm_default, ns_in_pm_default, position)
            VALUES (${newSec[0].id}, ${r.bus_number_default}, ${r.pickup_default}, ${r.dropoff_default},
              ${r.pickup_time_default}, ${r.ds_in_am_default}, ${r.ns_out_am_default}, ${r.ds_out_pm_default}, ${r.ns_in_pm_default}, ${j})
          `;
        }
      }
      return res.status(200).json({ ok:true });
