...
// Creates a Fleetio Work Order in "Open", maps Date/Odometer, attaches the PDF that is ALREADY on-screen
// (sent from the client as base64), and adds the "Schedule 4 Inspection (& EEPOC FMCSA 396.3)" Service Task.
...
// ---------- Work Order Number helpers ----------
function sanitizeWorkOrderNumber(n) {
  if (n == null) return null;
  const s = String(n).trim();
  // Typical API returns either integer or '#1234'. Drop leading '#' if present.
...
export default async function handler(req, res) {
  ...
  try {
    const {
      inspectionId,
      filename,
      reportUrl,           // optional now; we prefer pdfBase64 from client
      data = {},
      unitNumber,
      vehicleId: vehicleIdFromBody,
      serviceTaskName,
      pdfBase64            // <-- NEW: base64 for the on-screen PDF
    } = req.body || {};
    ...
    // 3) Create Work Order (v2) — issued/started from Date Inspected
    ...
    // Capture the human-friendly Work Order number for UI link
    ...
    const work_order_url = workOrderNumber
      ...
      : `https://secure.fleetio.com/${ACCOUNT_TOKEN}/work_orders/${workOrderId}/edit`; // fallback

    // 4) Build PDF buffer
    let pdfBuffer = null;
        // Ensure robust parsing of pdfBase64
    let raw = pdfBase64;
    if (raw && typeof raw === 'string') {
      ...
      try {
        ...
        const err = new Error('[decode_pdf] Invalid base64 payload for pdfBase64');
        err.step = 'decode_pdf'; err.status = 400;
        throw err;
      }
    }

    // If no pdfBase64 given, fall back to rendering server-side from reportUrl (with ?id=)
    if (!pdfBuffer) {
      if (!reportUrl) {
    ...
    // 5) Upload the PDF, then attach to WO
    ...
    // 6) Ensure Service Task exists (v1), then add as Work Order line item (v2)
    ...
    // 7) Explicitly create start & end Meter Entries on the Work Order so UI shows Start/Complete Odometer
    // (We do this in addition to ending_meter_same_as_start to guarantee both UI fields populate.)
    if (odometer != null) {
      ...
      await fleetioV1('/meter_entries', { method: 'POST', body: JSON.stringify(startBody) }, 'create_start_meter');
      ...
      await fleetioV1('/meter_entries', { method: 'POST', body: JSON.stringify(endBody) }, 'create_end_meter');
    }
