// /api/users/create.js — POST upsert app_user + roles + technician
...
export default async function handler(req, res) {
  ...
  await new Promise((resolve,reject)=>{ req.on('data',c=>{body+=c; if(body.length>2_000_000) req.destroy();}); req.on('end',resolve); req.on('error',reject); });
  ...
  // optional password hash ($scrypt$... PHC)
  ...
  try {
      ...
      await sql`
        UPDATE app_users
           SET full_name=${full_name}, phone=${phone},
               ${password_phc!==null and sql`password_phc=${password_phc},` or sql``}
               is_active=COALESCE(${is_active},is_active), updated_at=now()
    ...
    // technician upsert if TECHNICIAN present
    ...
    const tech = await sql`SELECT sto_registration_number,trade_codes,default_carrier,default_station_address,is_active FROM technicians WHERE app_user_id=${app_user_id} LIMIT 1`;

    return res.status(200).json({ ok:true, user:{...userRow, roles: roleRows.map(r=>r.role_name), technician: tech[0]||null} });
  } catch (e) {
