...
(function () {
  ...
  function createDebugModal(){
    ...
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.65);z-index:999999;display:flex;align-items:center;justify-content:center;';
    const modal = document.createElement('div');
    modal.style.cssText = 'width:min(900px,92vw);max-height:80vh;overflow:auto;background:#0b0b0c;color:#e5e7eb;border:1px solid #333;border-radius:12px;box-shadow:0 20px 70px rgba(0,0,0,.5)';
    modal.innerHTML = `
      <div style="padding:12px 14px;border-bottom:1px solid #222;display:flex;justify-content:space-between;align-items:center;">
        <div style="font-weight:600">Fleetio Debug</div>
        <button id="dbg-close" style="background:#333;color:#eee;border-radius:8px;padding:6px 10px;border:1px solid #444">Close</button>
      </div>
      <pre id="dbg-log" style="margin:0;padding:12px;white-space:pre-wrap;word-break:break-word;font:12px/1.45 ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace;"></pre>
    `;
    overlay.appendChild(modal); document.body.appendChild(overlay);
    const logEl = modal.querySelector('#dbg-log');
    function log(line){ try { logEl.textContent += (line + '\n'); } catch {} }
    modal.querySelector('#dbg-close').onclick = ()=> { try { document.body.removeChild(overlay); } catch {} };
    return { log, close: ()=>{ try { document.body.removeChild(overlay); } catch {} } };
  }
